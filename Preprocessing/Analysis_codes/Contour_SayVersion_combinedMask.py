...
# Callback function for the trackbar
    ...
    def compute_optical_flow(self, prev_frame, current_frame):
        ...
        blur = cv2.GaussianBlur(current_frame, (5, 5), 0)
        ...
        humans = human_cascade.detectMultiScale(blur, scaleFactor=1.1, minNeighbors=1, minSize=(30, 30))

        new_fgmask_curr = self.fgbg.apply(current_frame)
        combined_mask = cv2.bitwise_and(new_fgmask_curr, new_fgmask_curr, mask=cv2.bitwise_not(current_frame))

        # Create a mask for the detected human(s)
        # mask = np.zeros_like(gray)

        # for (x, y, w, h) in humans:
        #     # mask[y:y+h, x:x+w] = 255
        #     cv2.rectangle(combined_mask, (x, y), (x+w, y+h), (255, 255, 255), thickness=cv2.FILLED)
        # kernel = np.ones((5, 5), np.uint8)
        # mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel)
        # mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel)
        # Apply a bitwise AND operation to cancel out the background
        result = cv2.bitwise_and(current_frame, current_frame, mask=combined_mask)

        #original
        concatenated_frames2 = cv2.hconcat([current_frame, result])
        cv2.imshow('New', concatenated_frames2)
        cv2.waitKey(1)
