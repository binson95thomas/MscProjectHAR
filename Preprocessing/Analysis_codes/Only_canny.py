...
class OpticalFlowComputer:
    def __init__(self):
        self.fgbg = cv2.createBackgroundSubtractorMOG2(history=500,detectShadows=True)
...
# Callback function for the trackbar
    ...
    def compute_optical_flow(self, prev_frame, current_frame):
        ...
        fgmask_curr = self.fgbg.apply(prev_frame)
        ...
        edge_frame_curr_v1 =cv2.Canny(current_frame,lower, upper)
        edge_frame_prev_v1=cv2.Canny(prev_frame,lower, upper)

        fgmask_prev_v1 = self.fgbg.apply(current_frame)
        fgmask_curr_v1 = self.fgbg.apply(prev_frame)
        ...
        # V2 Original --> BGS --> Canny===================================================
        edge_frame_curr_v2 =cv2.Canny(current_frame_masked,lower, upper)
        edge_frame_prev_v2=cv2.Canny(prev_frame_masked,lower, upper)


        #original
        concatenated_frames = cv2.hconcat([current_frame, prev_frame_masked_v1,edge_frame_curr_v2])
        cv2.imshow('Original vs V1 vs V2', concatenated_frames)
        cv2.waitKey(1)
