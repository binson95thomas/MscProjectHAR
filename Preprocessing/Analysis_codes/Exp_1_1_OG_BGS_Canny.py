...
# Callback function for the trackbar
    ...
    def compute_optical_flow(self, prev_frame, current_frame):
        ...
        edge_frame_curr_v2 =cv2.Canny(current_frame_masked,65, 195)
        edge_frame_prev_v2=cv2.Canny(prev_frame_masked,65, 195)


        #original
        concatenated_frames = cv2.hconcat([current_frame, edge_frame_curr_v2])
        cv2.imshow('OriginalVs Preprocessed Frames', concatenated_frames)
        cv2.waitKey(1)
        

        prev_frame = cv2.medianBlur(edge_frame_prev_v2, 5)
        current_frame = cv2.medianBlur(edge_frame_curr_v2, 5)
        #====================================================================
