...
# Callback function for the trackbar
    ...
    def compute_optical_flow(self, prev_frame, current_frame):
        ...
        edge_fgmask_prev = self.fgbg.apply(edge_frame_curr)
        edge_fgmask_curr = self.fgbg.apply(edge_frame_prev)
        ...
        #original
        # concatenated_frames = cv2.hconcat([current_frame, edge_frame_curr,edge_current_frame_masked ])
        # cv2.imshow('Original vs Preprocessed Frame', concatenated_frames)
        cv2.imshow('Preprocessed Frame', edge_frame_curr)
        cv2.imshow('Masked Preprocessed Frame', edge_current_frame_masked)
        cv2.waitKey(1)
        ...
        flow = cv2.calcOpticalFlowFarneback(edge_current_frame_masked, edge_prev_frame_masked, None, 0.5, 3, 15, 3, 5, 1.2, 0)
        u_component = flow[..., 0]
        v_component = flow[..., 1]
