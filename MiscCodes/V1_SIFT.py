...
class OpticalFlowComputer:
    ...
    def compute_optical_flow(self, prev_frame, current_frame):
        ...
        #CAnny
        # canny_curr_frame= cv2.Canny(current_frame_masked,50,200)
        # ##laplacian
        # lap_curr_frame=  np.uint8(cv2.Laplacian(current_frame_masked,cv2.CV_64F))
        sift = cv2.SIFT_create()
        kp = sift.detect(current_frame, None)
 
        # Marking the keypoint on the image using circles
        SIFT_img=cv2.drawKeypoints(current_frame ,kp ,current_frame ,flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)

        edge_curr_frame= SIFT_img

        # concatenated_frames2 = cv2.hconcat([current_frame, edge_curr_frame])
        
        cv2.imshow('Original vs Edge  masked', SIFT_img)
        cv2.waitKey(1)
