...
class OpticalFlowComputer:
    ...
    def compute_optical_flow(self, prev_frame, current_frame):
        ...
        fgmask_prev = self.fgbg.apply(prev_frame)
        ...
        cv2.waitKey(1)
     #   v1 for comparison --Comments to be changed here
        #-------------------------canny-------------------------------

        thres=100
        thres_ratio=0.4
        canny_curr_frame =cv2.Canny(current_frame, thres * thres_ratio, thres, apertureSize=5)
        canny_prev_frame =cv2.Canny(prev_frame, thres * thres_ratio, thres, apertureSize=5)     
        
           #----------------------laplacian-------------------------------
        lap_curr_frame=  np.uint8(cv2.Laplacian(current_frame,cv2.CV_64F))
        lap_prev_frame=  np.uint8(cv2.Laplacian(prev_frame,cv2.CV_64F))
        #================================================================================

        edge_curr_frame = canny_curr_frame
        edge_prev_frame = canny_prev_frame
        #==================================================================================
        concatenated_filters2 = cv2.hconcat([current_frame,edge_curr_frame])
        cv2.imshow('Original Vs Edge', concatenated_filters2)
        cv2.waitKey(1)

        #Subtracting BG of Edge
        fgmask_prev = self.fgbg.apply(edge_prev_frame)
        fgmask_curr = self.fgbg.apply(edge_curr_frame)
        ...
        edge_prev_frame_masked = cv2.bitwise_and(edge_prev_frame, edge_prev_frame, mask=fgmask_prev)
        edge_curr_frame_masked = cv2.bitwise_and(edge_curr_frame, edge_curr_frame, mask=fgmask_curr)
        concatenated_frames2 = cv2.hconcat([edge_curr_frame, edge_curr_frame_masked])
        
        cv2.imshow('Edge vs BgSubEdge', concatenated_frames2)
        cv2.waitKey(1)
        #------------------Original Code----------------------------------
     ...
     # OPtical flow calc of Edge Detection

        edge_prev_frame = cv2.medianBlur(edge_prev_frame_masked, 5)
        edge_curr_frame = cv2.medianBlur(edge_curr_frame_masked, 5)
   
        edge_prev_blurred = cv2.GaussianBlur(edge_prev_frame, (5, 5), 0)
        edge_curr_blurred = cv2.GaussianBlur(edge_curr_frame, (5, 5), 0)
        
        edge_flow = cv2.calcOpticalFlowFarneback(edge_prev_blurred, edge_curr_blurred, None, 0.5, 3, 15, 3, 5, 1.2, 0)
        edge_u_component = edge_flow[..., 0]
        edge_v_component = edge_flow[..., 1]
        # Uncomment below to view optical flow as it runs
        edge_magnitude, edge_angle = cv2.cartToPolar(edge_u_component, edge_v_component, angleInDegrees=True)
        edge_hsv = np.zeros((edge_prev_frame.shape[0], edge_prev_frame.shape[1], 3), dtype=np.uint8)
        edge_hsv[..., 1] = 255
        edge_hsv[..., 0] = angle * 180 / np.pi / 2
        edge_hsv[..., 2] = cv2.normalize(edge_magnitude, None, 0, 255, cv2.NORM_MINMAX)
        edge_rgb = cv2.cvtColor(edge_hsv, cv2.COLOR_HSV2BGR)

        concatenated_OF = cv2.hconcat([rgb , edge_rgb])
        #--------------------------------------------------------    
        cv2.imshow('Optical Flow Original v/s Edge', concatenated_OF)
