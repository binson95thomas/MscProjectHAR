...
class OpticalFlowComputer:
    ...
    def compute_edge_detector(self, current_frame):
        resized_frame = cv2.resize(current_frame, (51, 38))
...
class OpticalFlowProcessor:
    ...
    def process_video(self, video_folder):
        ...
        while i < len(frames) - num_frames_in_window:
            ...
            ##########check this###############
            #######################################
            components_stacked_reshaped = components_stacked[..., np.newaxis]
            combined_array = np.concatenate((components_stacked_reshaped, combined_optical_flow), axis=-1)

            self.numpy_writer.write_array(combined_array, window_name)
...
if __name__ == "__main__":                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      
    try:
        ...
        output_folder = f"../Outputs/{model_type}/Unbalanced_combo_test"
