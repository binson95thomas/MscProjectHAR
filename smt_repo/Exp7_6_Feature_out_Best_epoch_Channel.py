...
class Raw_stream(nn.Module):
    ...
    def forward(self, x):
        # print(f"shape before Linear RAW: {x.shape}")
        ...
        # print(f"shape after pool RAW: {x.shape}")
        # x = x.view(x.size(0), -1)
        # x = F.relu(self.fc1(x))
        # x = self.dropout1(x)
        # x = F.relu(self.fc2(x))
...
class OF_Stream(nn.Module):
    ...
    def forward(self, x):
        # print(f"shape before OF RAW: {x.shape}")
        ...
        x = F.max_pool3d(x, 2)
        x = F.relu(self.bn4(self.conv4(x)))
        # print(f"shape before Linear OF: {x.shape}")
...
class TwoStreamConv(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        x = torch.cat((logits1, logits2), dim=1)# 1- Channel, 2-Spatial
        # print(f"shape after pool after cat: {x.shape}")
        ...
        # Apply fusion layer
        # fusion_logits = self.fc_fusion(x)
...
def train_model(dataloader_train1, dataloader_val1,dataloader_train2, dataloader_val2, num_epochs=50, learning_rate=0.001, weight_decay=1e-5):
    ...
    for epoch in range(num_epochs):
        ...
        if f1 > best_f1_score or (f1 == best_f1_score and accuracy > best_accuracy):
            ...
            best_epoch = epoch
            torch.save(model.state_dict(), f'{saved_model_path}_best_model.pth') 
            # torch.save(model.state_dict(), f'best_model_test.pth')
            print(f"New best model saved at epoch {epoch+1} with F1 Score: {best_f1_score:.4f} and Accuracy: {best_accuracy:.4f}")
            logging_output(f"New best model saved at epoch {epoch+1} with F1 Score: {best_f1_score:.4f} and Accuracy: {best_accuracy:.4f}", log_dir,log_name,False)
...
if __name__ == "__main__":
    ...
    try:
            ...
            model_sub_folder="CNN_256xFC_512_Channel_Orig"
            if lbool_model_sub:
            ...
            debug_log='debug.log'
            # Exp_6_1_Baseline_80_Split
            # Exp_6_2_BG+OF_Baseline_80_Split
            # Exp_6_3_Baseline_128x96_80_split
            
            prep_out_raw="Exp_6_1_Baseline_80_Split"
            pre_out_OF="Exp_6_2_BG+OF_Baseline_80_Split"
            ...
            if lbool_UH:
                ...
                Raw_stream_path = f"C:/Users/bt22aak/GPU_DS/{prep_out_raw}/balanced"
                OF_Stream_path = f"C:/Users/bt22aak/GPU_DS/{pre_out_OF}/balanced"

                Raw_test_path = f'C:/Users/bt22aak/GPU_DS/{prep_out_raw}/unbalanced'
                OF_Test_path = f'C:/Users/bt22aak/GPU_DS/{pre_out_OF}/unbalanced'
            else:

                Raw_stream_path = f"D:/UH_KTP/Outputs/{prep_out_raw}/balanced"
                OF_Stream_path = f"D:/UH_KTP/Outputs/{pre_out_OF}/balanced"

                Raw_test_path = f'D:/UH_KTP/Outputs/{prep_out_raw}/unbalanced'
                OF_Test_path = f'D:/UH_KTP/Outputs/{pre_out_OF}/unbalanced'

            logging_output(f"Sample raw path is  {Raw_stream_path}",log_dir,log_name)
            logging_output(f"Sample of path is  {OF_Stream_path}",log_dir,log_name)
            ...
            str_model_type=f'{name_only[0]}_{model_sub_folder}_b{batch_size}e{num_epochs}L{learning_rate}'
            ...
            try:
                model_summ = StringIO()
                sys.stdout = model_summ
                summary(model, [(1, 17, 38, 51), ( 2, 17, 38, 51)])
                str_model_Summ=model_summ.getvalue()
                logging_output(str_model_Summ,log_dir,'Model_summ.txt',False)
            except Exception as E :
            ...
            logging_output(f'Saved Model Path is {saved_model_path}',log_dir,log_name,True)
            if lbool_full_retrain:
                logging_output("Fully Retraining Model",log_dir,log_name)
                model = train_model(dataloader_train_raw, dataloader_val_raw,dataloader_train_OF, dataloader_val_OF, num_epochs,learning_rate)
            else:
                logging_output("Checking Trained Models",log_dir,log_name)

                if os.path.isfile(f"{saved_model_path}_best_model.pth"):
                    model.load_state_dict(torch.load(f"{saved_model_path}_best_model.pth", map_location=device))
                    logging_output(f"Loaded Best model.",log_dir,log_name)            

                elif os.path.isfile (f"{saved_model_path}"):
                    model.load_state_dict(torch.load(saved_model_path, map_location=device))
                    logging_output(f"Loaded saved model.",log_dir,log_name)

                elif os.path.isfile (f"{saved_model_path}_pre.pth"):
                    model.load_state_dict(torch.load(f"{saved_model_path}_best_model.pth", map_location=device))
                    logging_output(f"Loaded Pre model.",log_dir,log_name)            
