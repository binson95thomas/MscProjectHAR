# This code is using my model structure and not the say version
# Contains the split version
...
class CustomModel(nn.Module):
    ...
    def forward(self, x):
        x = self.pool1(self.norm1(torch.relu(self.conv1(x))))
        x = self.pool2(self.norm2(torch.relu(self.conv2(x))))
...
class FallDetectionCNN1(nn.Module):
    def __init__(self):
        super(FallDetectionCNN1, self).__init__()
...
class FallDetectionCNN2(nn.Module):
    def __init__(self):
        super(FallDetectionCNN2, self).__init__()
    ...
    def forward(self, x):
        ...
        return x
class FallDetectionCNN_Say(nn.Module):
    def __init__(self):
        super(FallDetectionCNN_Say, self).__init__()
...
class TwoStreamConv(nn.Module):
    def __init__(self):
        ...
        self.stream1 = FallDetectionCNN1()
        self.stream2 = FallDetectionCNN2()
...
def plot_metrics(accuracies, precisions, recalls, specificities, f1_scores, val_losses, train_losses):
    ...
    savepath=f'Results/TwoStr{model_folder1}/plt_{str_model_type}.png'
...
def plot_confusion_matrix(true_labels, predictions, classes):
    ...
    plt.savefig(f'./Results/TwoStr{model_folder1}/CM_{str_model_type}.png')
    # plt.show()
...
if __name__ == "__main__":
    try:
            ...
            model_folder1="Exp_6_1_Baseline_NoSplit"
            model_folder2="Exp_10_1_Contour_No_split"

            log_path=f'./Results/TwoStr{model_folder1}/TwoStreamTrainlog_UH.log'
            ...
            Raw_stream_path= f'C:\\Users\\bt22aak\\GPU_DS\\Exp_6_1_Baseline_No_split\\balanced'
            OF_Stream_path = f'C:\\Users\\bt22aak\\GPU_DS\Exp10_1_Contour_OF_No_split\\balanced'
            Raw_test_path = f'C:\\Users\\bt22aak\\GPU_DS\\{model_folder1}_No_split\\Unbalanced'
            OF_Test_path = f'C:\\Users\\bt22aak\\GPU_DS\\{model_folder2}_No_split\\Unbalanced'
            ...
            str_model_type=f'TwoStream_{model_folder1}_b{batch_size}e{num_epochs}L{learning_rate}'

            if not os.path.exists(f'./Results/TwoStr{model_folder1}'):
                try:
                    # Create the directory and its parents if they don't exist
                    os.makedirs(f'./Results/TwoStr{model_folder1}')
                except OSError as e:
                    print(f"Error creating directory ./Results/{model_folder1}: {e}")
            ...
            model = TwoStreamConv().to(device)
            saved_model_path = f'./Results/TwoStr{model_folder1}/{str_model_type}.pth'
    		
            if os.path.isfile(saved_model_path):
                model.load_state_dict(torch.load(saved_model_path, map_location=device))
                logging_output(f"Loaded saved model.",log_path)

            elif os.path.isfile (f"{saved_model_path}_pre"):
                model.load_state_dict(torch.load(f"{saved_model_path}_pre", map_location=device))
                logging_output(f"Loaded Pre-saved model.",log_path)
