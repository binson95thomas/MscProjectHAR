...
def plot_metrics(
    accuracies, precisions, recalls, specificities, f1_scores, val_losses, train_losses
):
    ...
    plt.figtext(
        x=0,
        y=0,
        s=f"{str_model_type}: {num_epochs} Epochs with learning rate {learning_rate}",
    )
...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        accuracy, precision, recall, specificity, f1 = compute_metrics(
        ...
        print(
            f"Epoch [{epoch+1}/{num_epochs}], Loss: {loss.item():.4f}, Accuracy: {accuracy:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, Specificity: {specificity:.4f}, F1-Score: {f1:.4f}"
        )
        logging_output(
            f"Epoch [{epoch+1}/{num_epochs}], Loss: {loss.item():.4f}, Accuracy: {accuracy:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, Specificity: {specificity:.4f}, F1-Score: {f1:.4f}",
            log_path,
...
if __name__ == "__main__":
    ...
    test_path = "../GPU_DS/ubl_BGS_Canny_dyn_76"
    batch_size = 32
    num_epochs = 50
    learning_rate = 0.000001
    str_model_type = f"ubl_bal_BGS+Canny(dyn)+size(102x76)_b{batch_size}e{num_epochs}L{learning_rate}"
    ...
    (
        ...
        true_labels,
        predictions,
    ) = evaluate_model(model, dataloader_test, criterion, device)

    print(
        f"Test Loss: {test_loss:.4f}, Test Accuracy: {test_accuracy:.4f}, Test Precision: {test_precision:.4f}, Test Recall: {test_recall:.4f}, Test Specificity: {test_specificity:.4f}, Test F1-Score: {test_f1:.4f}"
    )
    logging_output(
        f"Test Loss: {test_loss:.4f}, Test Accuracy: {test_accuracy:.4f}, Test Precision: {test_precision:.4f}, Test Recall: {test_recall:.4f}, Test Specificity: {test_specificity:.4f}, Test F1-Score: {test_f1:.4f}",
        log_path,
