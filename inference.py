...
fgbg = cv2.createBackgroundSubtractorMOG2(history=200, varThreshold=30, detectShadows=True)
fgbg.setShadowValue(0)
fgbg.setShadowThreshold(0.5)

def preprocess_frames(frame, fgbg, learning_rate = -1):
    normalized_frame = cv2.normalize(src = frame, dst = None, alpha = 0, beta = 255, norm_type = cv2.NORM_MINMAX, dtype = cv2.CV_8U)
    fgmask = fgbg.apply(normalized_frame, learningRate = learning_rate)
    
    kernel = np.ones((4,4), np.uint8)
    fgmask = cv2.morphologyEx(fgmask, cv2.MORPH_CLOSE, kernel)
    fgmask = cv2.morphologyEx(fgmask, cv2.MORPH_OPEN, kernel)
    
    masked_frame = cv2.bitwise_and(normalized_frame, normalized_frame, mask=fgmask)
...
def process_video(video_path, model, device, fgbg, window_size=18, overlap=9):
    ...
    while True:
        ...
        frame = preprocess_frames(frame, fgbg)
        cv2.imshow("current frame", frame)
        cv2.waitKey(1)

        if prev_frame is not None:
            u, v = compute_optical_flow(prev_frame, frame)
...
model = FallDetectionCNN().to(device)
model.load_state_dict(torch.load('fall_detection_model_3d_1to1.pth', map_location = device))
...
process_video(video_path, model, device, fgbg)