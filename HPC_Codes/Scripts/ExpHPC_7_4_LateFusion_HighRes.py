...
if __name__ == "__main__":
    try:
            ...
            pre_out_OF="Exp_6_4_OF_HighRes_128x96"
            ...
            logging_output(f"Preparing Model Summary",log_dir,log_name,True)
            try:
                model_summ = StringIO()
                sys.stdout = model_summ
                summary(model, [(1, 17, 128, 96), ( 2, 17, 128, 96)])
                str_model_Summ=model_summ.getvalue()
                logging_output(str_model_Summ,log_dir,'Model_summ.txt',False)
                logging_output(f"Model Summary Complete",log_dir,log_name,True)
