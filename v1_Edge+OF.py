...
class OpticalFlowComputer:
    ...
    def compute_optical_flow(self, prev_frame, current_frame):
        ...
        #This is a comparison of Original Image Vs Edge Detected image without Backgrond subtraction
        ...
        # v1 for comparison
        #canny
        canny_curr_frame =cv2.Canny(current_frame,50,200, apertureSize=7)
        canny_prev_frame =cv2.Canny(prev_frame,50,200, apertureSize=7)
        ...
        # Origianl  vs Canny
        concatenated_filters = cv2.hconcat([current_frame,canny_curr_frame])
        
        cv2.imshow('Original Vs Edge', concatenated_filters)
        cv2.waitKey(1)
        # Origianl  vs Lap

        # concatenated_filters2 = cv2.hconcat([current_frame,lap_curr_frame])
        
        # cv2.imshow('Original Vs Laplacian', concatenated_filters2)
        # cv2.waitKey(1)
        ...
        edge_prev_frame = cv2.medianBlur(canny_curr_frame, 5)
        edge_curr_frame = cv2.medianBlur(canny_prev_frame, 5)
        ...
        #--------------------------------------------------------
        cv2.imshow('Optical Flow comparison', concatenated_OF)
        cv2.waitKey(1)
